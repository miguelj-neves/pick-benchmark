...
def main(dataset_name, output, tasks, noise_before_events):
    np.random.seed(42)
    ...
    if "split" in dataset.metadata.columns:
        dataset.filter(dataset["split"].isin(["dev", "test", "TBD"]), inplace=True)

    dataset.preload_waveforms(pbar=True)

    if tasks[0]:
        generate_task1(dataset, output, noise_before_events)
    if tasks[1] or tasks[2]:
        generate_task23(dataset, output)


def generate_task1(dataset, output, noise_before_events):
    np.random.seed(42)
    windowlen = 30 * 100  # 30 s windows
    labels = []
    ...
    for i in tqdm(range(len(dataset)), total=len(dataset)):
        ...
        # Redefine sampling_rate from metadata for each sample
        sampling_rate = metadata.get("trace_sampling_rate_hz", 100)  # Default to 100 if not provided
        
        windowlen = 30 * sampling_rate
        ...
        if len(p_arrivals) == 0 and len(s_arrivals) == 0:
            ...
            if end_sample - start_sample <= windowlen:
                sample = {
                    ...
                    "start_sample": start_sample,
                    "end_sample": end_sample,
                    "trace_type": metadata["source_type"],
                }
                labels += [sample]
...
def generate_task23(dataset, output):
    np.random.seed(42)
    windowlen = 10 * 100  # 30 s windows
    labels = []
